...
def analyze_passes(satellites, lat, lon, radius, hours):
    ...
    # UTC 시간 리스트 생성
    utc_list = [t0 + step * i for i in range(int((t1 - t0).total_seconds() / 60) + 1)]
    times = [ts.utc(t.year, t.month, t.day, t.hour, t.minute) for t in utc_list]
            ...
            if dist <= radius:
                if not in_zone:
                    ...
                    entry = {
                        ...
                        "Start LH HorizVel (km/sec)": round(sat_obj.at(t).velocity.km_per_s[0], 3)
                    }
            else:
                if in_zone:
                    in_zone = False
                    geo2 = sat_obj.at(t).subpoint()
                    entry.update({
                        ...
                        "Stop LH HorizVel (km/sec)": round(sat_obj.at(t).velocity.km_per_s[0], 3),
                        "Duration (sec)": (i - entry["Start Index"]) * 60
                    })
                    del entry["Start Index"]  # CSV에서 제외
                    results.append(entry)

    return pd.DataFrame(results)

# 🎨 Streamlit UI
st.title("🛰️ 위성 범위 진입/이탈 분석 도구")
